...
da(1) = beta*((3*(x(1)+x(2))/2)^(4/3))*(a(2)-a(1)) + param.kappa*RCDE - param.gamma*RPQ; % change in propanal
...
for i = 2:xnum-1
    ...
    dp(i) = beta*(xhp*(p(i+1)-p(i))-xhm*(p(i)-p(i-1)))- RCDE; %change in 1,2-PD
    da(i) = beta*(xhp*(a(i+1)-a(i))-xhm*(a(i)-a(i-1)))+ param.kappa*RCDE - param.gamma*RPQ; %change in propanal
end
